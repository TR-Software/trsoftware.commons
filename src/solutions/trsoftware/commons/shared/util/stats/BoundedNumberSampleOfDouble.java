...
/**
 * Represents a fixed-size number sample of {@code double} values, and computes {@linkplain #summarize() statistics}
 ...
 */
public class BoundedNumberSampleOfDouble {
  ...
  /**
   * Creates a new instance backed by a {@code double[]} array of the given length.
   *
   * @param capacity the maximum number of values that can be held by this buffer;
   * {@linkplain #add(double) adding} a new value after {@link #size()} exceeds this capacity will
   * {@linkplain #evict(double) evict} the eldest value
   */
  public BoundedNumberSampleOfDouble(int capacity) {
    if (capacity <= 0) {
   ...
   */
  public BoundedNumberSampleOfDouble(BoundedNumberSampleOfDouble other) {
  ...
  /**
   * Adds a new value to the buffer, {@linkplain #evict(double) evicting} the eldest value if buffer capacity exceeded.
   ...
   */
  public Optional<SampleStatisticsDouble> summarize() {
   ...
   */
  public Optional<SampleStatisticsDouble> summarize(int fromIndex) {
   ...
   */
  public Optional<SampleStatisticsDouble> summarize(int fromIndex, int toIndex) {
    if (size > 0) {
      fromIndex = normalizePositionIndex(fromIndex, size, "fromIndex");
      toIndex = normalizePositionIndex(toIndex, size, "toIndex");
      if (toIndex - fromIndex > 0) {
        NumberSampleOnlineDouble stats = new NumberSampleOnlineDouble();
   ...
   * @throws IndexOutOfBoundsException if the index is less than {@code -size} or greater than {@code size}
   */
  public OptionalDouble median(int fromIndex) {
   ...
   * @throws IndexOutOfBoundsException if either index is less than {@code -size} or greater than {@code size}
   */
  public OptionalDouble median(int fromIndex, int toIndex) {
    if (size > 0) {
      double[] arr = toArray(fromIndex, toIndex);
   ...
   * @throws IndexOutOfBoundsException if the index is less than {@code -size} or greater than {@code size}
   */
  public double[] toArray(int fromIndex) {
...
//    Slice slice = new Slice(fromIndex, toIndex);
   ...
   * @throws IndexOutOfBoundsException if the index is less than {@code -size} or greater than {@code size}
   */
  public DoubleStream stream(int fromIndex) {
  ...
  public DoubleStream stream(int fromIndex, int toIndex) {
    fromIndex = normalizePositionIndex(fromIndex, size, "fromIndex");
    toIndex = normalizePositionIndex(toIndex, size, "toIndex");
    return IntStream.range(fromIndex, toIndex).mapToDouble(this::get);
  }
  ...
  /**
   * @return a <code>double[]</code> {@linkplain Doubles#asList(double...) array wrapper}
   * containing the values currently held in this buffer, ordered from eldest to most-recently added.
   */
  public List<Double> asList() {
    return Doubles.asList(toArray());
  }
  ...
  public boolean equals(Object o) {
    ...
    if (o == null || getClass() != o.getClass())
      return false;
    BoundedNumberSampleOfDouble that = (BoundedNumberSampleOfDouble)o;
