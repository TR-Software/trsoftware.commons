...
/**
 * Reads CSV-formatted input into String arrays representing the rows.
 *
...
public class CSVReader implements Closeable {
   ...
   *         entry, or null if no more lines available.
   * 
   * @throws IOException if bad things happen during the read
   */
   ...
   * @return the comma-tokenized list of elements, or null if nextLine is
   *         null
   * @throws IOException if bad things happen during the read
   */
      ...
      for (int i = 0; i < nextLine.length(); i++) {
        ...
        if (c == this.escape) {
          if (isEscapable(nextLine, inQuotes, i)) {
            ...
            sb.append(c);  // this char is not the start of an escape sequence (\\ or \"), so include it in the output
        }
        else if (c == quotechar) {
